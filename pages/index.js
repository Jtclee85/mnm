...
4.  **'교과평어 만들기' 요청:** 대화 내용 전체를 바탕으로, 학생의 탐구 과정, 질문 수준, 이해도, 태도 등을 종합하여 선생님께 제출할 수 있는 정성적인 '교과 세부능력 및 특기사항' 예시문을 2~3문장으로 작성해 줘. **다른 말 없이, 순수한 평가 내용만 <summary> 태그로 감싸서 출력해.**
      `
    };
  };
  ...
  const handleSpecialRequest = (prompt, title, metadata) => {
    if (isLoading) return;
  ...
  const handleRequestQuiz = () => {
      const prompt = "지금까지 대화한 내용을 바탕으로, 학습 퀴즈 1개를 내주고 나의 다음 답변을 채점해줘.";
      handleSpecialRequest(prompt, "좋아! 그럼 지금까지 배운 내용으로 퀴즈를 내볼게.", { type: 'quiz' });
  };
  const handleRequestThreeLineSummary = () => {
      const prompt = "내가 처음에 제공한 [원본 자료]의 핵심 내용을, 하나의 문단으로 자연스럽게 이어지는 3줄 정도 길이의 요약글로 생성해 줘. 제목이나 다른 말 없이, 순수한 요약 내용만 <summary> 태그로 감싸서 출력해.";
      handleSpecialRequest(prompt, "### 📜 3줄 요약", { type: 'summary' });
  };
  const handleRequestEvaluation = () => {
      const prompt = "지금까지 나와의 대화, 질문 수준 등을 바탕으로 나의 학습 태도와 이해도를 '나 어땠어?' 기준에 맞춰 평가해 줘.";
      handleSpecialRequest(prompt, "### 💯 나 어땠어?", { type: 'evaluation' });
  };
  const handleRequestTeacherComment = () => handleSpecialRequest("지금까지의 활동을 바탕으로 선생님께 보여드릴 '교과평어'를 만들어 줘.", "### ✍️ 교과평어 예시", { type: 'teacher_comment' });
            ...
            {m.role === 'assistant' && !isLoading && (
              <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                {m.metadata?.type === 'summary' && (
                 ...
                 {/* ✨ [수정됨] '교과평어 만들기' 버튼이 다시 보이도록 로직 복원 */}
