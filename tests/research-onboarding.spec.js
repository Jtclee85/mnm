...
test.describe('뭐냐면 — 자료 조사 나침반 / 조사 시작 전 퀘스트 (5차)', () => {
  ...
  test.describe('자료 조사 나침반 — 상시 체크리스트', () => {
    ...
    test('플로팅 챗봇 버튼과 겹치지 않는다', async ({ page }) => {
      await page.goto('/');
      const compassBox = await page.getByTestId('research-compass-toggle').boundingBox();
      const chatbotButton = page.locator('[aria-label*="챗봇"], [aria-label*="질문"]').first();
      const chatbotCount = await chatbotButton.count();
      if (chatbotCount > 0) {
        const chatbotBox = await chatbotButton.boundingBox();
        if (compassBox && chatbotBox) {
          const overlap =
            compassBox.x < chatbotBox.x + chatbotBox.width &&
            compassBox.x + compassBox.width > chatbotBox.x &&
            compassBox.y < chatbotBox.y + chatbotBox.height &&
            compassBox.y + compassBox.height > chatbotBox.y;
